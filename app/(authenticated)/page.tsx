...
export default function ProjectsPage() {
  ...
  const [showAll, setShowAll] = useState(true); // Alterna entre todos os projetos e projetos do usuário
  ...
  useEffect(() => {
    async function fetchProjects() {
      ...
      try {
        // Decide o endpoint com base no estado `showAll`
        ...
        // Recebe os dados e define os estados
        const data = await response.json();
        setProjects(data.projects || []); // Projetos
        setUserId(data.userId || null); // ID do usuário logado, se houver
      } catch (error: any) {
        console.error(error);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        {userId && ( // Mostra o botão de alternar apenas para usuários logados
          <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {projects.map((project) => {
          // Define se o usuário pode editar ou deletar o projeto
          const canEditOrDelete =
            userId && (project.createdBy === userId || project.developers.some((dev: any) => dev.id === userId));
